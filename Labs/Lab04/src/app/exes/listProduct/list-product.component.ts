...
export class ListProductComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // Theo dõi sự thay đổi của filterValue từ service
  ...
  set listFilter(value: string){  
    ...
    setTimeout(() => {
      this.filteredProducts = this.products.filter(p =>
        p.productName.toLowerCase().includes(this._filterValue)
      );
    },100);
  ...
  // handleRatingClicked(message: string): void {
  //   console.log('handleRatingClicked()- listProduct.component.ts is called', message); // Thêm dòng này
  //   console.log(message); // In thông báo ra console hoặc bạn có thể xử lý nó theo cách khác
  //   alert(message);
  // }
}
